...
class OAuthSignerSpec: QuickSpec {
    override func spec() {
        ...
        let requestForTest = TwitterOAuthEndpoint<OAuthRequestToken>(
            baseURL: .oauth,
        ...
        let signer = OAuthSigner(credential: credentialForTest,
        ...
        describe("create signatre key") {
            it("has credential and access token") {
                let signer = OAuthSigner(credential: credentialForTest, accessToken: accessTokenForTest)
            ...
            it("has only credential") {
                let signer = OAuthSigner(credential: credentialForTest, accessToken: nil)
        ...
        it("collect parameters") {
            ...
            expect(parameterString).to(equal(
        ...
        it("make signature base string") {
            ...
            expect(signatureBaseString).to(equal(
                "POST"
        ...
        it("sign request") {
            let urlRequest = signer.sign(to: requestForTest)

            let authorizationHeaderValue = urlRequest.allHTTPHeaderFields!["Authorization"]!

            expect(authorizationHeaderValue).to(equal(
                "OAuth "
                    + #"oauth_consumer_key="xvz1evFS4wEEPTGEFPHBog", "#
                    + #"oauth_nonce="kYjzVBB8Y0ZFabxSWbWovY3uYSQ2pTgmZeNu2VS4cg", "#
